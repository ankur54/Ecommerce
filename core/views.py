...
def shop_grid_new(request):
    ...
    if 'search' in request.GET and request.GET.get('search') is not None  :
        ...
        if 'color' in request.GET and 'category' not in request.GET:
            ...
            items=results.filter(
                color__color_name__in=filtered_colors).order_by(sort_by)

        if 'color' not in request.GET and 'category' in request.GET:
            print('77')
            filtered_colors=tuple(Item.objects.values(
                'color__color_name').distinct())
            filtered_categories=tuple(request.GET.getlist('category'))
            print('filtered_categories = ', filtered_categories)
            items=results.filter(
                category__name__in=filtered_categories).order_by(sort_by)
            ...
            print('filtered_categories = ', filtered_categories)
            print('filtered_colors = ', filtered_colors)
            items=results.filter(color__color_name__in=filtered_colors).filter(
                category__name__in=filtered_categories).order_by(sort_by)
        ...
        if 'color' not in request.GET and 'category' in request.GET:
            ...
            print('filtered_categories = ', filtered_categories)
            print('filtered_colors = ', filtered_colors)
            items=Item.objects.filter(color__color_name__in=filtered_colors).filter(
                category__name__in=filtered_categories).order_by(sort_by)
